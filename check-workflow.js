...
dqlTasks.forEach(t => {
  ...
  const hasDollarVar = /\$[A-Za-z]/.test(q);
  const hasTimeframe = wf.tasks[t].input.timeframe || null;
  console.log(`  ${t} | limit: ${hasLimit} | has$var: ${hasDollarVar} | timeframe: ${JSON.stringify(hasTimeframe)}`);
});
...
if (wf.tasks.overall_prompt) {
  ...
  console.log('predecessors:', op.predecessors);
  console.log('conditions:', JSON.stringify(op.conditions?.states));
  console.log('supplementary refs count:', (op.input.supplementary.match(/result\(/g) || []).length);
}

// Check for unbalanced parens/brackets in DQL
console.log('\n=== Balance check ===');
dqlTasks.forEach(t => {
  const q = wf.tasks[t].input.query;
